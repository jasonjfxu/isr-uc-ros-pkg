...
void robotDataCallback(std::string * data){ 
    ...
    int left_encoder_count, right_encoder_count, alcohol_sensor, dust_sensor, temp_max, ldr;
    sscanf(data->c_str(), "@25,%d,%d,%d,%d,%d,%de", &right_encoder_count, &left_encoder_count, &alcohol_sensor, &dust_sensor, &temp_max, &ldr);   //encoder msg parsing
    ...
    if (current_time.toSec() - 0.099 >= last_time_pub){
	publish_info = true;
    }
	...
	odom.twist.twist.angular.z = vel_yaw;
   	...
   	msg.x=alcohol_sensor;
   	msg.y=dust_sensor;
   	msg.z=temp_max;
   	
	std_msgs::Float32 msg_ldr;
	msg_ldr.data=ldr;
   	
	// Publish the message
	pub_LDRsensor.publish(msg_ldr);
	pub_sensors.publish(msg);
...
//  FILE *file;
//  file = fopen ("/home/mrl/Desktop/sensores.m","a");
...
int main(int argc, char** argv){ //typical usage: "./traxbot_node /dev/ttyACMx"
	...
	odom_broadcaster_ptr = &odom_broadcaster;
        pub_sensors = n.advertise<geometry_msgs::Vector3>("robot_sensors", 1);
	pub_LDRsensor = n.advertise<std_msgs::Float32>("/robot_ldr_sensor", 1);
	sub_class = n.subscribe("/sensors_classiffication", 1, classifficationCB);  
